        ...
        if individual:
            ...
            for i in range(len(atoms)):
                ...
                for type2 in atomic_types:
                    key = (type1,type2)
                    key2 = tuple(sorted(list(key)))
                    fingerprints[i][key] = self._get_atomic_rdf(feature[0][i][key2], nbins, m, smearing_norm, c)
                    fingerprints[i][key] /= self.binwidth1*atomic_count[atomic_types.index(type2)]/volume
                    fingerprints[i][key] -= 1
            ...
            for i in range(len(atoms)):
                ...
                for type2,type3 in type_combinations:
                    ...
                    fingerprints[i][key] = self._get_atomic_adf(feature[1][i][key], nbins, m, smearing_norm, c)
                    fingerprints[i][key] /= self.binwidth2*atomic_count[atomic_types.index(type2)]\
                                            *atomic_count[atomic_types.index(type3)]/volume
                    fingerprints[i][key] -= 1
