import numpy as np
from scipy.spatial.distance import euclidean
import matplotlib.pyplot as plt

from ase.io import read, write

import os

def collect_events(a_gm, runs_path, featureCalculator, dmax=0.1, N=100, traj_name=None, save_dir='stats', check_finalPop=False):
    f_gm = featureCalculator.get_feature(a_gm)
    Nruns = 0
    times = []
    events = []
    structures = []
    iter_max = 0
    for i in range(N):
        found = False
        try:
            if check_finalPop:
                traj_temp = read(runs_path + 'run{}/global{}_finalPop.traj'.format(i,i), index=':')
            traj = read(runs_path + 'run{}/global{}_spTrain.traj'.format(i,i), index=':')
            Nruns += 1
            run_length = len(traj)
            if run_length > iter_max:
                iter_max = len(traj)

            fMat = featureCalculator.get_featureMat(traj)
            for i, (f, a) in enumerate(zip(fMat, traj)):
                d = euclidean(f, f_gm)
                if d < dmax:
                    times.append(i)
                    events.append(1)
                    structures.append(a)
                    found = True
                    break
            if not found:
                times.append(run_length)
                events.append(0)            
        except:
            pass
        

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if traj_name is not None:
        write(save_dir+'/'+traj_name, structures)

    return times, events

def collect_events_energy(a_gm, runs_path, dE_max=0.3, N=100, traj_name=None, save_dir='stats', check_finalPop=False):
    E_gm = a_gm.get_potential_energy()
    Nruns = 0
    times = []
    events = []
    structures = []
    iter_max = 0
    for i in range(N):
        found = False
        try:
            if check_finalPop:
                traj_temp = read(runs_path + 'run{}/global{}_finalPop.traj'.format(i,i), index=':')
            traj = read(runs_path + 'run{}/global{}_spTrain.traj'.format(i,i), index=':')
            Nruns += 1
            run_length = len(traj)
            if run_length > iter_max:
                iter_max = len(traj)

            E_all = np.array([a.get_potential_energy() for a in traj])
            for i, (E, a) in enumerate(zip(E_all, traj)):
                if E < E_gm + dE_max:
                    print(i, E)
                    times.append(i)
                    events.append(1)
                    structures.append(a)
                    found = True
                    break
            if not found:
                times.append(run_length)
                events.append(0)            
        except:
            pass
        

    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    if traj_name is not None:
        write(save_dir+'/'+traj_name, structures)

    return times, events

