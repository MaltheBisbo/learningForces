...
from searchStat.survival_statistics.kernelregression import KernelRegression # This should point to your kernel ridge modul
...
def survival_stats(times,events,alpha=0.95,sigma=5000,show_plot=True,save=True,get_hazard=True,labels=[], save_dir='stats'):
    ...
    if save: # Save files
        ...
        for i in range(n_inputs):
            ...
            if os.path.isfile(os.path.join(cwd,save_dir,name_survival+'.npy')):
                n_name = 1
                while os.path.isfile(os.path.join(cwd,save_dir,name_survival+'({})'.format(n_name)+'.npy')):
                    n_name += 1
            ...
            if os.path.isfile(os.path.join(cwd,save_dir,name_CH+'.npy')):
                n_name = 1
                while os.path.isfile(os.path.join(cwd,save_dir,name_CH+'({})'.format(n_name)+'.npy')):
                    n_name += 1
                name_CH += '({})'.format(n_name)
            name_CH += '.npy'
            np.save(os.path.join(cwd,save_dir,name_CH),(NA[i][0],NA[i][1],NA[i][2],NA[i][3],censoring[i]))

            if get_hazard:
                name_H = name+'_Haz'
                if os.path.isfile(os.path.join(cwd,save_dir,name_H+'.npy')):
                    n_name = 1
                    while os.path.isfile(os.path.join(cwd,save_dir,name_H+'({})'.format(n_name)+'.npy')):
                        n_name += 1
                    name_H += '({})'.format(n_name)
                name_H += '.npy'
                np.save(os.path.join(cwd,save_dir,name_H),(Hazard[i][0],Hazard[i][1]))
