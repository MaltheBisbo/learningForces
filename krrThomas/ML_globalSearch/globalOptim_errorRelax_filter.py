...
class globalOptim():
    ...
    def runOptimizer(self):
        ...
        for i in range(self.Niter):
            ...
            t1_all = time()
            #self.update_MLrelaxed_pop()
            if Enew < np.mean(self.MLmodel.data_values):
    ...
    def newCandidate_beyes(self):
        ...
        if self.master:
            ...
            # Filter too uncertain candidates
            index_all = np.arange(len(fitness_all))
            uncertainty_filter = error_all < 0.95*np.sqrt(np.abs(self.MLmodel.theta0))
            fitness_filtered = fitness_all[uncertainty_filter]
            index_filtered = index_all[uncertainty_filter]
            
            index_best = index_filtered[fitness_filtered.argmin()]
