...
class globalOptim():
    ...
    """
    def __init__(self, traj_namebase, MLmodel, startGenerator, mutationSelector, calc=None, startStructures=None, population_size=5, kappa=2, Niter=50, Ninit=2, sigma=20, Ncand_min=30, min_saveDifference=0.3, minSampleStep=10, dualPoint=False, relaxFinalPop=False):
        ...
        for i in range(self.Niter):
            ...
            for i in range(5):
                ...
                try:
                    ...
                    kappa0 /=2
                    print('Error in singlePoint cought:', runtime_err, flush=True)
            t_sp_end = time()
    ...
    def relaxTrue(self, a):
        ...
        a_relaxed, Erelaxed, Frelaxed = relaxGPAW(a, label, calc=self.calc)
    ...
    def singlePoint(self, anew):
        ...
        if self.master:
            self.writer_spPredict.write(anew)

        write('structure_sp_waiting.traj', anew.copy())
        self.comm.barrier()
        run_module('single_gpaw')
        self.comm.barrier()
        
        a = read('structure_sp_done.traj')
        E = a.get_potential_energy()
        F = a.get_forces()

        # Delete temporary trajectories
        if self.master:
            os.remove('structure_sp_waiting.traj')
            os.remove('structure_sp_done.traj')
        self.comm.barrier()
