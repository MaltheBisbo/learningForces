...
class globalOptim_baseClass():
    ...
    def runOptimizer(self):
        ...
        for i in range(self.Niter):
            ...
            self.update_MLrelaxed_pop(prior_fitness=prior_fitness)
            
            # Generate new rattled + MLrelaxed candidate
            t_newCand_start = time()
            a_all, a_mutated_all, E_all, error_all = self.newCandidate_beyes(prior_fitness=prior_fitness)
            t_newCand_end = time()
    ...
    def update_MLrelaxed_pop(self, prior_fitness=False):
