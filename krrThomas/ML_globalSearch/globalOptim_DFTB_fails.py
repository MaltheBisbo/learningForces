...
class globalOptim():
    ...
    """
    def __init__(self, traj_namebase, MLmodel, startGenerator, mutationSelector, calc=None, startStructures=None, population_size=5, kappa=2, Niter=50, Ninit=2, sigma=20, reg=1e-5, Ncand_min=30, min_certainty=1.0, dualPoint=False, relaxFinalPop=False):
    ...
    def update_MLrelaxed_pop(self):
        label = self.pop_dir + 'ML_pop{}'.format(self.traj_counter)
        write(label+'.traj', self.population.pop)
    ...
    def trainModel(self):
        ...
        GSkwargs = {'reg': [self.reg], 'sigma': self.sigma}  # SnO
