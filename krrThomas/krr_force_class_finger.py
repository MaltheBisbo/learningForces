...
class krr_force_class():
    ...
    **comparator_kwargs:
    ...
    def fit(self, forceMat, positionMat, featureMat=None, reg=1e-3):
        ...
        self.featureGrad = self.featureCalculator.get_gradientMat(self.positionMat)
    ...
    def predict_force(self, pos_new, fnew=None):
        if fnew is None:
            assert self.featureCalculator is not None
            fnew = self.featureCalculator.get_singleFeature(pos_new)

        (Ndata, Ncoord) = self.positionMat.shape
        featureGrad_new = self.featureCalculator.get_singleGradient(pos_new)
    ...
    def predict_energy(self, pos_new, fnew=None):
        if fnew is None:
            assert self.featureCalculator is not None
            fnew = self.featureCalculator.get_singleFeature(pos_new)
