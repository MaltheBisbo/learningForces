...
class krr():
    ...
    def plotModel(self, x_min, x_max):
        ...
        for x0, alpha in zip(self.x_train, self.alpha):
            ...
            d = np.abs(x_plot - x0)
            y = alpha*np.exp(-d**2/(2*self.sigma**2)) + self.beta
            #plt.plot(x_plot,y, 'k:', lw=1.5)
            plt.fill_between(x_plot, self.beta*np.ones(len(x_plot)), y, facecolor='blue', alpha=0.2)
...
# train model
beta = -0.5
model.train(x_train, y_train, beta=beta)
...
# plot bias line
#plt.plot([0,10], [beta,beta], 'k--', lw=2)

plt.legend(loc=3, fontsize=fs)
plt.savefig('figures/krrExample6.pdf', transparent=True)
plt.show()
