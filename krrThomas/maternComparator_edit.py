...
class maternComparator():
    ...
    def get_similarity_vector(self, f, featureMat=None, split=False):
        if featureMat is not None:
            self.featureMat = featureMat
        v = self.n + 0.5
        ...
        if not split:
            return (k1 * k2).reshape(-1)
        else:
            return k1, k2, d

    def single_comparison(self, f1, f2, sigma=None, split=False):
        if sigma is None:
            sigma = self.sigma
        v = self.n + 0.5
        c = np.sqrt(2*v)/self.sigma
    ...
    def get_jac(self, f, featureMat):
        ...
        """
        k1Vec, k2Vec, dVec = self.get_similarity_vector(f, featureMat, split=True)
        v = self.n + 0.5
        c = np.sqrt(2*v)/self.sigma
    ...
    def get_jac_new(self, f1, f2):
        ...
        Using the chain rule: dk_df = dk_dd*dd_df , where d is the distance measure
        """
        k1, k2, d = self.single_comparison(f1, f2, split=True)
        if d == 0:
...
if __name__ == '__main__':
    ...
    for i, x in enumerate(x_array):
        ...
        kernel_Jac[i] = matern.get_jac_new(x1,x0)
