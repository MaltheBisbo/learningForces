...
class gaussComparator_local():
    def __init__(self, Natoms, **kwargs):
        self.Natoms = Natoms
        if 'sigma' in kwargs:
    ...
    def get_kernelVec(self, fnew, featureMat):
        """
        fnew has shape (Natoms, Nelements_feature)
        featureMat has shape (Ndata*Natoms, Ndata*Natoms)
        """
        N_localFeatures = len(featureMat)
        Ndata = int(N_localFeatures/self.Natoms)
        I = np.identity(Ndata)
