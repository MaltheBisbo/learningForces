...
class delta_krr():
    ...
    def energy(self, atoms, feature=None):
        return self.MLmodel.predict_energy(atoms, fnew=feature)

    def forces(self, atoms, feature=None):
        return self.MLmodel.predict_force(atoms, fnew=feature)
...
class krr_class():
    ...
    def __init__(self, comparator, featureCalculator, delta_function=None, bias_func=None, regGS=None, sigmaGS=None):
        ...
        if isinstance(delta_function, krr_class):
            # wrap krr_class into the right delta_function format
            self.delta_function = delta_krr(delta_function)
            self.bias_func = None
            self.using_krr_prior = True
        else:
            self.delta_function = delta_function
            self.bias_func = bias_func
            self.using_krr_prior = False
    ...
    def predict_energy(self, atoms=None, fnew=None, similarityVec=None, delta_values=None, return_error=False, no_prior_on_error=False):
        ...
        if delta_values is None:
            if self.delta_function is not None:
                if self.using_krr_prior and return_error:
                    delta, delta_err, _ = self.delta_function.MLmodel.predict_energy(atoms, return_error=True)
                else:
        ...
        if return_error:
            ...
            if self.using_krr_prior and not no_prior_on_error:
                frac = prediction_error/np.sqrt(np.abs(self.theta0))
                mixing = frac
                prediction_error = (1-mixing)*prediction_error + mixing*delta_err
    ...
    def predict_force(self, atoms=None, fnew=None, fgrad=None, return_error=False):
        ...
        if self.delta_function is not None:
            if self.using_krr_prior and return_error:
                delta_force, delta_force_err = self.delta_function.MLmodel.predict_force(atoms, return_error=True)
            else:
        ...
        if return_error:
            ...
            if self.using_krr_prior:
                _, prediction_error, _ = self.predict_energy(atoms, return_error=True, no_prior_on_error=True)
                frac = prediction_error/np.sqrt(np.abs(self.theta0))
                mixing = frac
                error_force = (1-mixing)*error_force + mixing*delta_force_err
    ...
    def train(self, atoms_list=None, data_values=None, features=None, delta_values=None, add_new_data=True, k=3,
        ...
        **GSkwargs:
        ...
        if regGS is not None:
            self.regGS = regGS
        if sigmaGS is not None:
            self.sigmaGS = sigmaGS
        assert self.regGS is not None and self.sigmaGS is not None
        ...
        if delta_values is None:
            if self.delta_function is not None:
                if self.using_krr_prior:
                    delta_values = np.array([self.delta_function.energy(a, feature=f) for a,f in zip(atoms_list,features)])
    ...
    def __gridSearch(self, data_values, featureMat, k, delta_values=None):
        ...
        if len(reg_array) < 2 and len(sigma_array) < 2 and k==0:
            self.reg = reg_array[0]
            self.sigma = sigma_array[0]
            ...
            return None, {'sigma': self.sigma, 'reg': self.reg}
