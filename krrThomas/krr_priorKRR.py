...
class krr_class():
        ...
        if delta_values is None:
            if self.delta_function is not None:
                if self.using_krr_prior and return_error:
                    delta, delta_err, _ = self.delta_function.MLmodel.predict_energy(atoms, fnew=fnew, return_error=True)
                else:
    ...
    def predict_force(self, atoms=None, fnew=None, fgrad=None, return_error=False):
        ...
        if self.delta_function is not None:
            if self.using_krr_prior and return_error:
                delta_force, delta_force_err = self.delta_function.MLmodel.predict_force(atoms,
                                                                                         fnew=fnew,
                                                                                         fgrad=fgrad,
                                                                                         return_error=True)
            else:
