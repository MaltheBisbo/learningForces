...
class Angular_Fingerprint_local(object):
    ...
    def get_feature(self, atoms):
        """
        Returns 2D array with the local feature for all atoms in the structure.
        The dimension of a feature is thus:
        dim = Natoms x Nelements,
        where 'Nelements' is the number of elements in one local feature.
        """
        ...
        feature1 = <double*>mem.alloc(Natoms * Nelements_2body, sizeof(double))
        ...
        for i in range(Natoms):
            for j in range(Natoms):
                ...
                for n in range(minbin_lim, maxbin_lim + 1):
                    ...
                    feature1[j*Nbins1 + newbin] += value

        # Convert radial feature to numpy array
        feature1_np = np.zeros((Natoms, Nelements_2body))
        for i_atom in range(Natoms):
            for m in range(Nelements_2body):
                feature1_np[i_atom, m] = feature1[i_atom*Nbins1 + m]
