...
def relax_VarianceBreak(structure, calc, label, niter_max=1, steps=500):
    '''
...
def rattle_Natoms2d_center(struct, Nrattle, rmax_rattle=3.0, Ntries=10):
    ...
    rmin = 0.85  # 0.7*2*cov_radii
    rmax = 1.8  # 1.4*2*cov_radii
...
class globalOptim():
    ...
    def trainModel(self):
        ...
        #GSkwargs = {'reg': np.logspace(-7, -7, 1), 'sigma': [10]}
    ...
    def relax(self, a, ML=False):
        ...
        if ML:
            ...
            a_relaxed = relax_VarianceBreak(a, krr_calc, label, niter_max=1, steps=100)
            #a.set_calculator(krr_calc)
            #dyn = BFGS(a, trajectory=label+'.traj')
            #dyn.run(fmax=0.1)            
        else:
            label = self.traj_namebase + '{}'.format(self.traj_counter)
            a_relaxed = relax_VarianceBreak(a, self.calculator, label, niter_max=1, steps=100)
...
if __name__ == '__main__':
    ...
    try:
        ...
        run_num = ''
    savefiles_namebase = savefiles_path + '/' 
