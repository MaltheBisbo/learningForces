...
class krr_class():
    ...
    """
    def __init__(self, comparator, comparator_fine, featureCalculator, frac_comparator=0.1, delta_function=None, bias_func=None, regGS=None, sigmaGS=None):
        self.featureCalculator = featureCalculator
        self.comparator = comparator
        self.comparator_fine = comparator_fine
        self.frac_comp = frac_comparator
        self.regGS = regGS
    ...
    def predict_energy(self, atoms=None, fnew=None, similarityVec=None, delta_values=None, return_error=False, no_prior_on_error=False):
        ...
        if similarityVec is None:
            ...
            similarityVec = (1-self.frac_comp)*self.comparator.get_similarity_vector(fnew, self.featureMat) + self.frac_comp*self.comparator_fine.get_similarity_vector(fnew, self.featureMat)
    ...
    def predict_force(self, atoms=None, fnew=None, fgrad=None, return_error=False):
        ...
        dk_df = (1-self.frac_comp)*self.comparator.get_jac(fnew, featureMat=self.featureMat) + self.frac_comp*self.comparator_fine.get_jac(fnew, featureMat=self.featureMat)
    ...
    def __gridSearch(self, data_values, featureMat, k, delta_values=None):
        ...
        similarityMat = (1-self.frac_comp)*self.comparator.get_similarity_matrix(featureMat) + self.frac_comp*self.comparator_fine.get_similarity_matrix(featureMat)
