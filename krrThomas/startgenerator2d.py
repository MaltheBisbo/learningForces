...
def random_pos(box):
    ...
    xlen = vspan[0][0]
    ylen = vspan[1][1]
    r = np.c_[np.random.random((1, len(vspan))) - 0.5, 0].reshape(-1) * np.array([xlen,ylen,0])
    pos = center + r
    return pos
...
class StartGenerator(object):
    ...
    def __init__(self, slab, atom_numbers,
                 closest_allowed_distances, plane_to_place_in, elliptic=False, cluster=True):
        self.slab = slab
        self.atom_numbers = atom_numbers
        self.blmin = closest_allowed_distances
        self.plane = plane_to_place_in
            ...
            for i in range(N):
                ...
                while not pos_found:
                    if self.elliptic == True:
                        pi = random_pos_elliptic(self.plane)
                    else:
                        pi = random_pos(self.plane)
                    if i == 0:
                        break
