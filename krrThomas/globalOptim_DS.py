...
class globalOptim():
    ...
    def __init__(self, calculator, traj_namebase, MLmodel=None, Natoms=6, Niter=50, rattle_maxDist=0.5, kbT=1, Nstag=10, min_saveDifference=0.3, minSampleStep=10, MLerrorMargin=0.1, Nstart_pop=5, maxNtrain=1.5e3, fracPerturb=0.3, noZ=False):
        ...
        open(traj_namebase + 'MLerror.txt', 'a').close()
    ...
    def doubleShot(self, a_unrelaxed, Nshot):
        ...
        for i_shot in range(Nshot):
            ...
            # Relax with MLmodel
            a_new, EnewML = self.relaxML(a_unrelaxed)
    ...
    def relaxML(self, a, label=None):
        ...
        # Save prediction error of ML-relaxed structure
        Epred, error, theta0 = self.MLmodel.predict_energy(a_relaxed, return_error=True)
